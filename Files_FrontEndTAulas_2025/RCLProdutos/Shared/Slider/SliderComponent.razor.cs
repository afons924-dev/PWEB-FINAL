...
namespace RCLProdutos.Shared.Slider
{
    public partial class SliderComponent
    {
        ...
        [Inject]
        public IFavoritosTemporariosServices favoritosTemporariosServices { get; set; }
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            try
            {
                ...
                // Sincronizar favoritos através do servico
                await favoritosTemporariosServices.SincronizarFavoritosComAPI(_apiServices, await userSessionService.IsUserLoggedIn());

                // ASSINALAR PRODUTOS FAVORITOS POR CAUSA DO CORACAO
                var favoritosAtualizados = await favoritosTemporariosServices.GetProdutosFavoritos();

                foreach (var produto in produtos)
                {
                    produto.Favorito = favoritosAtualizados.Any(f => f.ProdutoId == produto.Id);
                }
